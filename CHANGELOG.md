...
**Providers**
- VirtualBox provider *(experimental)* — manage Oracle VM VirtualBox VMs via `VBoxManage` over SSH; long-format `VBoxManage list -l vms` listing (the only single-call shape that surfaces per-VM state), ACPI-graceful `acpipowerbutton` stop with `poweroff` escalation (mirrors libvirt's posture), `savestate` suspend, snapshots, and the `--type headless` start that keeps VMs from hanging on a missing display
